...
import io.prestosql.plugin.omniscidb.OmniSciDBConfig.ArrayMapping;
...
import static io.prestosql.plugin.omniscidb.OmniSciDBConfig.ArrayMapping.AS_ARRAY;
import static io.prestosql.plugin.omniscidb.OmniSciDBConfig.ArrayMapping.AS_JSON;
import static io.prestosql.plugin.omniscidb.OmniSciDBConfig.ArrayMapping.DISABLED;
import static io.prestosql.plugin.omniscidb.OmniSciDBSessionProperties.getArrayMapping;
import static io.prestosql.plugin.omniscidb.TypeUtils.arrayDepth;
import static io.prestosql.plugin.omniscidb.TypeUtils.getArrayElementPgTypeName;
import static io.prestosql.plugin.omniscidb.TypeUtils.getJdbcObjectArray;
...
public class OmniSciDBClient
        extends BaseJdbcClient
{
    private static final Logger log = Logger.get(OmniSciDBClient.class);
    ...
    @Inject
    public OmniSciDBClient(
            BaseJdbcConfig config,
            OmniSciDBConfig omniSciDBConfig,
            ...
            TypeManager typeManager)
    {
        ...
        if (omniSciDBConfig.isIncludeSystemTables()) {
        ...
        this.aggregateFunctionRewriter = new AggregateFunctionRewriter(
                ...
                ImmutableSet.<AggregateFunctionRule>builder()
                        ...
                        .add(new ImplementSum(OmniSciDBClient::toTypeHandle))
    ...
    protected void renameTable(JdbcIdentity identity, String catalogName, String schemaName, String tableName, SchemaTableName newTable)
    {
        if (!schemaName.equals(newTable.getSchemaName())) {
            throw new PrestoException(NOT_SUPPORTED, "Table rename across schemas is not supported in OmniSciDB");
        }
...
//    }
    ...
    public Optional<ColumnMapping> toPrestoType(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        switch (jdbcTypeName) {
            ...
            case "timestamptz":
                // OmniSciDB's "timestamp with time zone" is reported as Types.TIMESTAMP rather than Types.TIMESTAMP_WITH_TIMEZONE
        ...
        if (typeHandle.getJdbcType() == Types.TIME) {
            // When inserting a time such as 12:34:56.999, OmniSciDB returns 12:34:56.999999999. If we use rounding semantics, the time turns into 00:00:00.000 when
        ...
        if (typeHandle.getJdbcType() == Types.NUMERIC && getDecimalRounding(session) == ALLOW_OVERFLOW) {
            if (typeHandle.getColumnSize() == 131089) {
                ...
                // 131089 = SELECT LENGTH(pow(10::numeric,131071)::varchar); 131071 = 2^17-1  (org.omniscidb.jdbc.TypeInfoCache#getDisplaySize)
        ...
        // TODO support OmniSciDB's TIME WITH TIME ZONE explicitly, otherwise predicate pushdown for these types may be incorrect
    ...
    private Optional<ColumnMapping> arrayToPrestoType(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        if (baseElementTypeHandle.getJdbcType() == Types.BINARY) {
            // OmniSciDB jdbc driver doesn't currently support array of varbinary (bytea[])
    ...
    public WriteMapping toWriteMapping(ConnectorSession session, Type type)
    {
        ...
        if (type instanceof TimestampWithTimeZoneType && ((TimestampWithTimeZoneType) type).getPrecision() <= MAX_SUPPORTED_TIMESTAMP_PRECISION) {
            ...
            String omnisciType = format("timestamptz(%d)", precision);
            if (precision <= TimestampWithTimeZoneType.MAX_SHORT_PRECISION) {
                return WriteMapping.longMapping(omnisciType, shortTimestampWithTimeZoneWriteFunction());
            }
            else {
                return WriteMapping.objectMapping(omnisciType, longTimestampWithTimeZoneWriteFunction());
            }
        }
    ...
    private static LongWriteFunction timestampWriteFunction(TimestampType timestampType)
    {
        return (statement, index, value) -> {
            ...
            statement.setObject(index, localDateTime/*toPgTimestamp(localDateTime)*/);
        };
    }
    ...
    private static LongReadFunction shortTimestampWithTimeZoneReadFunction()
    {
        return (resultSet, columnIndex) -> {
            // OmniSciDB does not store zone information in "timestamp with time zone" data type
    ...
    private static JdbcTypeHandle getArrayElementTypeHandle(Connection connection, JdbcTypeHandle arrayTypeHandle)
    {
        ...
        try {
            Map<String, Class<?>> typeInfo = connection.unwrap(OmniSciConnection.class).getTypeMap();
...
//            TypeInfo typeInfo = connection.unwrap(OmniSciConnection.class).getTypeInfo();
...
//                    arrayTypeHandle.getArrayDimensions());
    ...
    {
        return (statement, index, value) -> {
            Map<String, String> pgObject = new HashMap<>();
            pgObject.put(jdbcTypeName, value.toStringUtf8());
...
//            statement.setObject(index, pgObject);
    ...
    {
        /*
         * OmniSciDB JDBC maps "money" to Types.DOUBLE, but fails to retrieve double value for amounts
         ...
         *
         * The following mapping maps OmniSciDB "money" to Presto "varchar".
         * Writing is disabled for simplicity.
         *
         * Money mapping can be improved when OmniSciDB JDBC gains explicit money type support.
