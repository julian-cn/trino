...
final class TypeUtils
{
    ...
    // OmniSciDB jdbc array element names understood by org.omniscidb.jdbc2.TypeInfoCache#getPGArrayType
    // for multidimensional arrays, this should return the base type (e.g. array(array(integer)) returns 'integer')
    static String getArrayElementPgTypeName(ConnectorSession session, OmniSciDBClient client, Type elementType)
    {
        ...
        // Since OmniSciDB char[] can only hold single byte elements, map all CharType to varchar
            ...
            throws SQLException
    {
        ...
        // OmniSciDB requires subarrays with matching dimensions, including arrays of null
        if (elementType instanceof ArrayType) {
            ...
            throws SQLException
    {
        ...
        if (prestoType instanceof TimestampWithTimeZoneType) {
            // OmniSciDB does not store zone, only the point in time
    ...
    static Map<String, String> toPgTimestamp(LocalDateTime localDateTime)
            throws SQLException
    {
        Map<String, String> pgObject = new HashMap<>();
        pgObject.put("timestamp", localDateTime.toString());
        return pgObject;
    }
}
